...
Morph, fontHeight, StageMorph, SyntaxElementMorph*/

// Global stuff ////////////////////////////////////////////////////////

modules.byob = '2012-August-08';
    ...
    if (!this.isPrototype) {
        ...
        this.inputs().forEach(function (inp) {
            if (inp instanceof TemplateSlotMorph) {
                ...
                up = inp.template();
                up.fixBlockColor(); // don't forget zebra coloring
                up.fixLabelColor();
            }
            idx += 1;
        });
...
CustomCommandBlockMorph.prototype.refreshPrototype = function (editorsOnly) {
    ...
    // determine the targetSet
    targetSet = editorsOnly ?
            myself.receiver().allEditorBlockInstances(myself.definition)
                : myself.receiver().allBlockInstances(myself.definition);

    // update the targetSet of blocks
    targetSet.forEach(
        function (eachBlock) {
            eachBlock.setCategory(myself.category);
            if (!editorsOnly || (eachBlock.blockSpec !== blockSpec)) {
                oldInputs = eachBlock.inputs();
                eachBlock.setSpec(blockSpec);
                eachBlock.restoreInputs(oldInputs);
                i = 0;
            ...
            eachBlock.fixBlockColor(null, true);
            eachBlock.fixLabelColor();
        }
    );
};
...
BlockEditorMorph.prototype.updateDefinition = function () {
    ...
    if (head) {
        ...
        head.children[0].refreshPrototype(true); // refresh blocks in editors
    }
...
InputSlotDialogMorph.prototype.fixSlotsLayout = function () {
	var slots = this.slots,
        xPadding = 10,
        ypadding = 14,
        bh = fontHeight(10) + 15, // slot type button height
        ah = fontHeight(10) + 10, // arity button height
